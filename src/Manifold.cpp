...
Manifold::Manifold(Soldier *p_Soldier, Bomb *p_Bomb, std::vector<sf::Vector2f> &p_CollisionNormals, float p_Penetration) {
	for (const auto &i : p_CollisionNormals) {
		m_FinalCollisionNormal.x += i.x;
		m_FinalCollisionNormal.y += i.y;
	}
	...
	sf::Vector2f correction = m_FinalCollisionNormal * p_Penetration;
	p_Soldier->getPosition() = (p_Soldier->getPosition() - correction);
	p_Bomb->getPosition() = (p_Bomb->getPosition() + correction);
}
